...
class CExplorationStateManager extends CSelfUpdatingComponent
{
	...
	private			 			var		m_TeleportedFallBlockTime		: float; 
	private	editable 			var		m_TeleportedFallBlockTimeTotalF	: float; 			default	m_TeleportedFallBlockTimeTotalF	= 0.1f;
	...
	private function SuperStateChanged( stateExiting, stateEntering : name )
	{
		...
		if( stateExiting == 'TraverseExploration' )
		{
			...
			{
				m_TeleportedFallBlockTime	= m_TeleportedFallBlockTimeTotalF;
			}
		}
	...
	private function UpdateExternalData( _Dt : float )
	{
		...
		if( m_TeleportedFallBlockTime > 0.0f )
		{
			return;
		}
