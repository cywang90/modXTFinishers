...
class CR4LocomotionPlayerControllerScript extends CR4LocomotionDirectControllerScript
{
	...
	function UpdateLocomotion()
	{
		...
		var inputInCameraSpace, inputAngleToRotate, previousSpeed : float;		
		var playerYaw				: float;
		...
		if ( !player.CanUpdateMovement() )
		{
			...
			player.SetBehaviorVariable( 'playerSpeedForOverlay', moveSpeed );
		...
		else
		{
			...
			player.SetBehaviorVariable( 'playerSpeedForOverlay', movingAgentComponent.GetRelativeMoveSpeed());
