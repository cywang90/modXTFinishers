...
state MeditationWaiting in W3PlayerWitcher extends MeditationBase
{
	...
	event OnLeaveState( nextStateName : name )
	{
		...
		if(abortRequested)
		{
			LatentMeditationWaitingAbort();
		}
		
		super.OnLeaveState(nextStateName);
	}
	
	entry function LatentMeditationWaitingAbort()
	{
	...
	private entry function Loop_StupidNameCollision()
	{
		...
		GetWitcherPlayer().MeditationRestoring( GetSimulateBuffTime() );				
	...
	private final function GetSimulateBuffTime() : float
	{
