...
class W3Action_Attack extends W3DamageAction
{
	...
	public function GetPowerStatValue() : SAbilityAttributeValue
	{
		...
		{		
			...
			if(SkillNameToEnum(attackTypeName) == S_Sword_s02)
			{
				holdRatio = witcherAttacker.GetSpecialAttackTimeRatio();
				max = witcherAttacker.GetSkillAttributeValue(S_Sword_s02, PowerStatEnumToName(CPS_AttackPower), false, true);
